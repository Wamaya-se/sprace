...
export async function generateMetadata(): Promise<Metadata> {
	...
	return {
		title: t('forCreatorsTitle'),
		description: t('forCreatorsDescription'),
		openGraph: {
			title: t('forCreatorsTitle'),
			description: t('forCreatorsDescription'),
			type: 'website',
...
export default async function ForCreatorsPage() {
	...
	const earnings = [
		{
			title: t('earningsExample1Title'),
			range: t('earningsExample1Range'),
			note: t('earningsExample1Note'),
		},
		{
			title: t('earningsExample2Title'),
			range: t('earningsExample2Range'),
			note: t('earningsExample2Note'),
		},
		{
			title: t('earningsExample3Title'),
			range: t('earningsExample3Range'),
			note: t('earningsExample3Note'),
		},
	]
	...
	return (
		<>
					...
					<div className="flex flex-col items-start gap-8">
						...
						<div className="hero-animate hero-animate-delay-4 flex flex-col gap-4 sm:flex-row">
							<Button asChild size="lg">
								<Link href="/register/creator">{t('heroCtaPrimary')}</Link>
							</Button>
			...
			<section
				...
				aria-labelledby="for-creators-benefits"
			>
				<div className="mx-auto max-w-7xl px-6">
					<ScrollReveal className="mb-16 max-w-2xl">
						<h2
							id="for-creators-benefits"
			...
			{/* Flow */}
			<section className="bg-surface py-32" aria-labelledby="for-creators-flow">
				<div className="mx-auto max-w-5xl px-6">
					<ScrollReveal className="mb-16 text-center">
						<h2
							id="for-creators-flow"
							className="font-heading text-4xl font-bold tracking-[-0.03em] text-foreground md:text-5xl"
						>
							{t('flowTitle')}
						</h2>
			...
			<section
				className="bg-surface-container-low py-32"
				aria-labelledby="for-creators-earnings"
			>
				<div className="mx-auto max-w-7xl px-6">
					<ScrollReveal className="mb-16 max-w-2xl">
						<h2
							id="for-creators-earnings"
							className="font-heading text-4xl font-bold tracking-[-0.03em] text-foreground md:text-5xl"
						>
							{t('earningsTitle')}
						</h2>
							...
							<ScrollReveal key={item.title} delay={i * 100}>
								<Card className="h-full bg-surface-container p-8">
									<CardContent className="p-0">
										...
										<p className="mt-3 font-heading text-3xl font-bold tracking-[-0.03em] text-foreground">
											{item.range}
										</p>
										<p className="mt-3 font-sans text-sm leading-[1.7] text-muted-foreground">
											{item.note}
					...
					<ScrollReveal className="mt-10">
						<p className="font-sans text-xs leading-[1.7] text-muted-foreground">
			...
			<section className="relative overflow-hidden py-32">
				...
				<ScrollReveal className="relative mx-auto max-w-3xl px-6 text-center">
					...
					<div className="mt-10 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
						<Button asChild size="lg">
							<Link href="/register/creator">{t('ctaPrimary')}</Link>
						</Button>
						<Button asChild variant="secondary" size="lg">
