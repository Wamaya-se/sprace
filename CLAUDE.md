...
- **`unstable_cache` + `revalidateTag` on public routes:** Public marketing pages must cache expensive reads with a stable cache key + tag. Mutations that affect those reads must call `revalidateTag('tag', 'max')` (Next 16 requires the profile argument). **Inside `unstable_cache` scopes, reads MUST use `createPublicClient()` from `@/lib/supabase/public`** — the cookie-bearing `createClient()` from `@/lib/supabase/server` triggers a Next.js 16 build error because cached functions may not read dynamic sources like `cookies()`.
